...
**English** · [Deutsch](../../entwicklung/release-prozess.md)
...
5. **Maintain the compendium** — *mandatory at every release from v1.4.2*: changed
   endpoints → `referenz/api.md`; changed behaviour/model → the affected page under
   `verstehen/`; a new/changed view → `referenz/ansichten.md` + a new screenshot in
   `ui/screenshots/` (English in `ui/screenshots/en/`); migration notes on a
   data-model change in `referenz/datenmodell.md`; a new setting →
   `referenz/einstellungen.md` (the test requires every key).
...
|---|---|
| a new/changed endpoint | `referenz/api.md` |
| a changed calculation/data model | the matching page under `verstehen/` + possibly `referenz/datenmodell.md` |
| a new setting | `referenz/einstellungen.md` |
| a new utility | a new page under `verstehen/`, the index, architecture |
| a new/changed view | `referenz/ansichten.md` + a screenshot under `ui/screenshots/` (a real capture with demo data, English under `ui/screenshots/en/`) |
| a new question or fault | `einstieg/faq.md` or `betrieb/fehlersuche.md` |
...
- **Composed keys escape a literal check (v2.13.0).** `` t(`glossary.${id}.term`) ``
  ...
  settings fields).
- **Documentation without a test rots (v2.14.0).** Up to v2.13 no test checked
  the documentation below README and INSTALL. The index promised "all 68
  endpoints" when there were 86, and guides walked through menu items under
  names the app had not used for releases ("merge wizard", "Analysis → heating
  signature"). `DocsIntegrityTest` now checks links, anchors, mirrors, redirects, the
  index and the settings reference against `SettingsService::DEFAULTS`. It
  compares the path segment by segment with the directory listing:
  `file_exists` would have passed every link with the wrong upper/lower case on
  the Mac, and such a link is dead on GitHub.
- **Moving means redirecting (v2.14.0).** 61 files moved. Issues, forum posts and
  bookmarks of people you cannot reach point to the old paths. Every old file
  stays as a short redirect; the test forbids the documentation itself to link
  one of them — otherwise the transition becomes permanent.
- **A documented command must have run once (v2.14.0).** The installation guide
  recommended `python3 scripts/init_data.py --help`. The script knew no options:
  the call started the import and wrote to `./data`. Now `--help` answers before
  any import, and the script is marked as deprecated.
- **A translation needs a glossary (v2.14.0).** In the English catalogue the
  unit price was called "working price" and "energy price", the standing charge
  "base price" and "base charge" — four words for two things, and the
  documentation used yet others. Now it is "unit price" and "standing charge"
  everywhere, as on British bills. The same for counts: "1 meters" and
  "group(s)" are not a translation; counts go through `Intl.PluralRules`; the
