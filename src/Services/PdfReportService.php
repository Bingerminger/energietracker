...
final class PdfReportService
{
    ...
    public function build(int $year): string
    {
        ...
        $pdf->text(self::M, 120, 'Jahresbericht ' . $year, 30, true, [255, 255, 255]);
        $pdf->text(self::M, 150, $loc, 12, false, [255, 255, 255]);
        $pdf->text(self::M, 240, 'Erstellt am ' . date('d.m.Y'), 10, false, self::MUTE);
        ...
        $pdf->addPage();
        $y = $this->header($pdf, 'Übersicht ' . $year, $W);
        ...
        if (!empty($perSource)) {
            if (count($perSource) === 1) {
                ...
                $pdf->text(self::M + 16, $y + 24, 'Effizienzklasse · ' . (string)$s['label'], 11, false, self::MUTE);
                ...
                $pdf->textRight($W - self::M - 16, $y + 52, sprintf('Wohnfläche %d m²', (int)$eff['wohnflaeche_m2']), 9, false, self::MUTE);
                $y += 90;
            } else {
                $pdf->text(self::M, $y + 8, 'Effizienz je Heizquelle (kWh/m²·a)', 12, true, self::INK);
                $y += 28;
                foreach ($perSource as $s) {
                    $pdf->text(self::M + 8, $y, (string)$s['label'], 10, false, self::INK);
                    $pdf->text(self::M + 180, $y, 'Klasse ' . (string)($s['class'] ?? '–'), 10, true, self::ACCENT);
            ...
            $pdf->text(self::M, $y + 20, (string)($eff['note'] ?? 'Keine Effizienzdaten.'), 10, false, self::MUTE);
            $y += 50;
        }

        // Kosten/CO2-Übersicht je aktiver Utility
        $pdf->text(self::M, $y, 'Verbrauch & Kosten', 12, true, self::INK);
        $y += 24;
        $cols = [self::M, self::M + 160, self::M + 290, self::M + 410];
        $pdf->text($cols[0], $y, 'Verbrauchsart', 9, true, self::MUTE);
        $pdf->text($cols[1], $y, 'Verbrauch', 9, true, self::MUTE);
        $pdf->text($cols[2], $y, 'Kosten', 9, true, self::MUTE);
        $pdf->text($cols[3], $y, 'CO₂', 9, true, self::MUTE);
        $y += 6;
        ...
        foreach ($this->activeUtilities() as $utility) {
            ...
            $pdf->text($cols[0], $y, $u['label'], 10, false, self::INK);
        ...
        $pdf->addPage();
        $y = $this->header($pdf, 'Empfehlungen', $W);
        $recs = $this->recommendations->all();
        if (empty($recs)) {
            $pdf->text(self::M, $y + 20, 'Keine offenen Empfehlungen — alles im grünen Bereich.', 10, false, self::MUTE);
        } else {
            $y += 14;
            foreach (array_slice($recs, 0, 12) as $r) {
                if ($y > 780) { $pdf->addPage(); $y = $this->header($pdf, 'Empfehlungen (Forts.)', $W) + 14; }
    ...
    private function utilityPage(PdfWriter $pdf, string $utility, array $meter, array $monthly, int $year, float $W): void
    {
        $u = Utilities::get($utility);
        $pdf->addPage();
        $y = $this->header($pdf, $u['label'] . ' ' . $year . ' — ' . (string)($meter['name'] ?? ''), $W);
        ...
        $kpis = [
            ['Jahresverbrauch', $nf($sum, $isKwh ? 0 : 1) . ' ' . $unit],
            ['Ø pro Monat',     $nf($avg, $isKwh ? 0 : 1) . ' ' . $unit],
            ['Gesamtkosten',    $nf($cost, 2) . ' €'],
            ['Stärkster Monat', $maxM ? ((string)$maxM['ym'] . ' · ' . $nf($maxM[$vf], $isKwh ? 0 : 1)) : '–'],
            ['Schwächster Monat', $minM ? ((string)$minM['ym'] . ' · ' . $nf($minM[$vf], $isKwh ? 0 : 1)) : '–'],
        ];
        $ky = $y + 6;
        ...
        // Monatstabelle
        $headers = ['Monat', $isKwh ? 'kWh' : 'm³', 'Kosten €', 'ø Temp', 'HGT'];
        $hasWa = $this->anyKey($monthly, 'weather_adjusted');
        if ($hasWa) { $headers[] = 'wetterber.'; $headers[] = 'Δ%'; }
        $colX = [self::M];
        $cwTab = $W - 2 * self::M;
        ...
        foreach ($monthly as $m) {
            if ($y > 790) { $pdf->addPage(); $y = $this->header($pdf, $u['label'] . ' (Forts.)', $W) + 14; }
            $row = [
                (string)($m['ym'] ?? ''),
